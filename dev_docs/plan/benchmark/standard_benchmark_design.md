...
- AI2D / Video-MME / StreamingBench / visual_cache：用于视觉 KV 稀疏化。
...
7. 如果声明视觉稀疏化，增加 Video-MME、StreamingBench、visual_cache。
...
| 优先级 | 类型 | 例子 | 使用原则 |
| --- | --- | --- | --- |
| P0 | 仓库已有、已能产出 raw/per-sample/aggregate 的入口 | microbench、NIAH、LongBench、SCBench preprocessed、MathBench、multimodal visual_cache | 默认优先使用，作为开发迭代主路径 |
| P1 | 仓库已有但需要补记录字段或数据准备的入口 | 原始 SCBench、Video-MME、StreamingBench、AI2D | 可用于相关功能，但先补齐 run_info 和 per-sample 记录 |
