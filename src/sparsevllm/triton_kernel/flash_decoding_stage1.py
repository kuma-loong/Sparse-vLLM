...
def _fwd_kernel_flash_decode_stage1(
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        # Use float32 for dot product accumulation to improve precision
        att_value = tl.sum(q[None, :].to(tl.float32) * k.to(tl.float32), 1)
        att_value *= sm_scale
...
def _fwd_kernel_flash_decode_stage1_with_score(
    ...
    for start_n in range(0, block_n_size, 1):
        ...
        # Dot product with higher precision
        att_value = tl.sum(q[None, :].to(tl.float32) * k.to(tl.float32), 1)
        att_value = tl.where(offs_n_new < cur_batch_end_index, att_value, float("-inf"))
        
        tl.store(Attn_Score + cur_batch * stride_asbs + cur_head * stride_ash + offs_n_new * stride_asl,
...
def flash_decode_stage1(
    ...
    mid_out, mid_out_logsumexp, block_seq,
):
    BLOCK_SEQ = block_seq
    BLOCK_N = 16
    ...
    _fwd_kernel_flash_decode_stage1[grid](
        q, k, v, sm_scale, Req_to_tokens, B_req_idx, B_Seqlen, mid_out, mid_out_logsumexp,
        ...
        gqa_group_size, BLOCK_SEQ=BLOCK_SEQ, BLOCK_DMODEL=Lk, BLOCK_N=BLOCK_N,
