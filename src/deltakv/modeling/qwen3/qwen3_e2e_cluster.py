...
from transformers.models.qwen3.modeling_qwen3 import (
    Qwen3Attention, Cache, Unpack, FlashAttentionKwargs, apply_rotary_pos_emb,  # noqa
    Callable, eager_attention_forward, ALL_ATTENTION_FUNCTIONS, Qwen3DecoderLayer, Qwen3Model, Qwen3ForCausalLM,  # noqa
...
from deltakv.modeling.qwen3.qwen3_e2e import create_compressor, reshape_and_apply_qk_norm
...
class Qwen3AttnKVClusterCompress(Qwen3Attention):
...
class Qwen3LayerKVClusterCompress(Qwen3DecoderLayer):
    def __init__(self, config: KVQwen3Config, layer_idx: int):
        super().__init__(config, layer_idx)
        self.self_attn = Qwen3AttnKVClusterCompress(config=config, layer_idx=layer_idx)


class Qwen3ModelKVClusterCompress(Qwen3Model):
    def __init__(self, config: KVQwen3Config):
        super().__init__(config)
        self.layers = nn.ModuleList(
            [Qwen3LayerKVClusterCompress(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.post_init()


class Qwen3KVClusterCompress(Qwen3ForCausalLM):
    def __init__(self, config: KVQwen3Config):
        super().__init__(config)
        self.model = Qwen3ModelKVClusterCompress(config)
        self.mse = nn.MSELoss()
        self.post_init()
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if labels is not None:
            ...
            for n, mod in self.named_modules():
                if isinstance(mod, Qwen3AttnKVClusterCompress):
