...
def forward(
    ...
    **kwargs,
    ...
    if isinstance(self.token_budget, float) and self.token_budget < 1.0:
        token_budget = max(1, int(kv_seq_len * self.token_budget))
    else:
...
def enable_quest_attention_eval(model, args):
    ...
    def _enable_recursive(module):
        ...
        for name, child in reversed(module._modules.items()):
            ...
            if isinstance(child, (LlamaAttention, MistralAttention)):
                current_layer_id -= 1
                child.layer_id = current_layer_id
