...
class DeltaKVGreedySearch(GreedySearch):
    def __init__(self, model, tokenizer, copy_on_gpu=False):
        super().__init__(model, tokenizer)
        self.backup_state = None
        # 优先级：启动参数 > 环境变量 > 默认 False (CPU)
        self.copy_on_gpu = copy_on_gpu or os.environ.get("COPY_ON_GPU", "0") == "1"

    def _get_state_snapshot(self, cache, to_cpu=True):
        """枚举 cache 的所有属性，自动处理 Tensor 及其容器地设备迁移"""
        # 如果 self.copy_on_gpu 为 True，则强制不迁移到 CPU
        actual_to_cpu = False if self.copy_on_gpu else to_cpu
        device = "cpu" if actual_to_cpu else self.model.device

        def _recursive_snapshot(obj):
            if isinstance(obj, torch.Tensor):
                # detach() 移除计算图，to(device) 迁移设备
                res = obj.detach().to(device)
                # 如果设备没变（即留在 GPU 上），必须 clone() 才能保证是数据地物理副本而非引用
                if not actual_to_cpu:
                    res = res.clone()
                return res
        ...
        # 如果 cache 本身就是 list 或 tuple (例如 Quest 启用了 tuple_kv_cache)
        if isinstance(cache, (list, tuple)):
            return _recursive_snapshot(cache)
    ...
    def _apply_state_snapshot(self, cache, snapshot):
        ...
        # 如果 snapshot 是 list 或 tuple，说明 cache 也是不可变容器，需要返回新对象
        if isinstance(snapshot, (list, tuple)):
            return _recursive_restore(snapshot)

        for k, v in snapshot.items():
            val = _recursive_restore(v)
            # 优先尝试原地更新 dict 和 list 以保持引用稳定性
            if isinstance(val, dict):
                target = getattr(cache, k, None)
    ...
    def _decode(
        ...
        # 打印输入的最后 200 个 token 的内容
        print(f"\n--- [Decode Start] Last 200 tokens of input: ---\n{self.tokenizer.decode(input_ids[0, -200:], skip_special_tokens=False)}\n--- [End of Input Tail] ---\n")

        end_token_ids = (
                extra_end_token_ids
                + [self.tokenizer.eos_token_id]
                + (self.model.config.eos_token_id if isinstance(self.model.config.eos_token_id, list) else [self.model.config.eos_token_id])
        )
        ...
        for i in range(max_length):
            if i == 0:  # prefilling
                ...
                logits, self.past_kv = out.logits, out.past_key_values

                # 如果指定了拍快照（通常用于普通多轮对话的每轮回滚）
                if take_snapshot:
                    self.backup_state = self._get_state_snapshot(self.past_kv, to_cpu=True)
            ...
            if word.item() in end_token_ids or i == max_length - 1:
                # 记录：如果没到 EOS 且是 Self-Gen 模式，下一轮需要补 EOS
                if word.item() not in end_token_ids:
                    self.add_eos_to_next_prompt = True
                break
    ...
    def test(self, example, max_length=100, disable_golden_context=False):
        ...
        for idx, prompt in enumerate(example["prompts"]):
            ...
            # 处理 disable_golden_context 模式下的占位符
            if idx > 0 and disable_golden_context:
                # 移除 eval_utils 格式化产生的 "None"
                prompt = prompt.replace("None", "", 1)
            ...
            with torch.inference_mode():
                # 调用我们重写的 _decode。只有在 Golden Context 模式且不是最后一轮时才需要拍快照以供回滚
                result = self._decode(
                    input_ids,
                    max_length=max_length_per_turn,
                    dense_prefix=(idx > 0),
                    take_snapshot=(not disable_golden_context and idx < len(example["prompts"]) - 1)
                )

                answer = self.tokenizer.decode(result[0, len(input_ids[0]):], skip_special_tokens=True)
                results.append(answer)

                # 如果是 Golden Context 模式，回退到本轮 Prefill 结束的状态
                if not disable_golden_context and idx < len(example["prompts"]) - 1:
                    self.past_kv = self._apply_state_snapshot(self.past_kv, self.backup_state)

                # 如果是 Self-Gen 模式且生成没完（没到 EOS），通知下一轮补一个 EOS
                # 逻辑复用自 GreedySearch 基类的 add_eos_to_next_prompt
                # 注意：我们在 _decode 内部需要根据 word.item() 设置这个 flag
    ...
    def test_scdq(self, example, max_length=100):
        ...
        for idx, prompt in enumerate(example["prompts"]):
            ...
            if idx == 0:
                ...
                input_ids = model_inputs["input_ids"]
                with torch.inference_mode():
                    # 针对 SCDQ，第一轮只做 Prefill 并拍一个长久的快照
                    self._encode(input_ids, max_length=max_length_per_turn)
                self.backup_state = self._get_state_snapshot(self.past_kv, to_cpu=True)
            else:
                # 每一轮新 Query 开始前，恢复那个长久的快照
                self.past_kv = self._apply_state_snapshot(self.past_kv, self.backup_state)
                ...
                with torch.inference_mode():
                    # 这里不需要拍新快照，因为我们要保留那个 idx=0 的 Context 快照
                    result = self._decode(
                        input_ids,
                        max_length=max_length_per_turn,
                        dense_prefix=True,
                    ...
                    results.append(
                        self.tokenizer.decode(result[0, len(input_ids[0]):], skip_special_tokens=True)
                    )
            torch.cuda.empty_cache()
        self.clear()
        output = {"answers": results, "gt": example["ground_truth"]}
