...
RUN_ID="${RUN_ID:-hf_deltakv_no_kivi_no_fp8ref_topratio0p11_latentgs32_stride0_hotpotqa_smoke_gpu4_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  conda run -n svllm python -u benchmark/long_bench/pred.py \
    --model qwen25-7b-hf-deltakv-cq-no-kivi-no-fp8ref-topratio0p11-latentgs32-stride0-smoke \
