...
CONFIG_PATH="configs/qwen2_5_7b_hf_kivi2_group32_residual32.json"
MODEL_PATH="/data2/haojitai/models/Qwen2.5-7B-Instruct-1M"
RUN_ID="${RUN_ID:-qwen25_7b_hf_kivi2_hotpotqa_smoke_gpu4_$(date +%Y%m%d_%H%M%S)}"
...

{
  ...
  echo "kivi_quant_bits=2"
  echo "group_size=32"
  echo "residual_length=32"
  echo "start_time=$(date -Is)"

  conda run -n svllm python -u benchmark/long_bench/pred.py \
    --model qwen25-7b-hf-kivi2-smoke \
